-- Google Sheet is the canonical VMP source.
-- The first successful run backs up and removes the legacy dataset completely,
-- then loads the Sheet snapshot atomically. Later runs keep the same exact-set
-- semantics while preserving rows that still exist in the Sheet.

alter table public.vmp_sheet_sync_backups
  drop constraint if exists vmp_sheet_sync_backups_dataset_check;

alter table public.vmp_sheet_sync_backups
  add constraint vmp_sheet_sync_backups_dataset_check
  check (dataset in (
    'vmp_plan_items',
    'vmp_objects',
    'data_quality_issues',
...
revoke all on function public.rpc_sync_vmp_sheet_snapshot(text, text, text, jsonb, jsonb)
  from public, anon, authenticated;
revoke all on function public.rpc_rollback_vmp_sheet_sync(uuid)
  from public, anon, authenticated;
grant execute on function public.rpc_sync_vmp_sheet_snapshot(text, text, text, jsonb, jsonb)
  to service_role;
...
comment on function public.rpc_sync_vmp_sheet_snapshot(text, text, text, jsonb, jsonb)
  is 'Atomically replaces legacy VMP data on first canonical Sheet sync, then maintains an exact Sheet-owned set with backups.';
comment on function public.rpc_rollback_vmp_sheet_sync(uuid)
  is 'Exactly restores VMP domain and dependent data captured before a canonical Sheet sync run.';
