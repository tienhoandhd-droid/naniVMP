-- =====================================================================
-- VMP_SYNC_ROW_GUARD: thông báo lỗi tự chẩn đoán
--
-- Bối cảnh: 2026-07-08 → 2026-07-29 sync đứng 21 ngày. Guard chặn đúng
-- (Sheet bị dán trùng 21 lần: 9.724 dòng / 461 ID duy nhất) nhưng chỉ in
--   "source row count 9724 is outside 450..5000"
-- Câu này khiến người đọc tưởng phải NỚI NGƯỠNG, trong khi việc đúng là
-- DỌN SHEET. Migration này thêm số ID duy nhất + tỉ lệ lặp + hướng xử lý
-- vào thông báo, để chẩn đoán được ngay từ mail cảnh báo mà không cần mở DB.
--
-- Chỉ đổi phần thông báo lỗi. Ngưỡng 450..5000 GIỮ NGUYÊN, logic giữ nguyên.
-- Chẩn đoán chỉ chạy khi sắp raise → đường chạy bình thường không tốn thêm.
-- =====================================================================
...
begin
  ...
  if v_source_rows < 450 or v_source_rows > 5000 then
    -- Chỉ tính chẩn đoán khi sắp raise -> đường chạy bình thường không tốn thêm gì.
    select count(distinct public.vmp_sheet_value(entry -> 'values', 16))
      into v_diag_ids
    from jsonb_array_elements(p_rows) as x(entry);

    v_diag_ratio := round(v_source_rows::numeric / nullif(v_diag_ids, 0), 1);

    raise exception
      'VMP_SYNC_ROW_GUARD: % dong / % ID duy nhat (ti le lap %x) - ngoai khoang 450..5000. %',
      v_source_rows,
      coalesce(v_diag_ids, 0),
      coalesce(v_diag_ratio, 0),
      case
        when coalesce(v_diag_ratio, 0) > 1.1
          then 'Sheet bi dan trung: DON SHEET, KHONG noi nguong guard.'
        when v_source_rows > 5000
          then 'Sheet phinh that su: kiem tra nguon truoc khi doi nguong.'
        else 'Sheet thieu dong: kiem tra quyen chia se va dung tab chua.'
      end;
  end if;
