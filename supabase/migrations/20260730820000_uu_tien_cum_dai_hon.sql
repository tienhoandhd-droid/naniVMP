-- =====================================================================
-- Cụm dài hơn thì cụ thể hơn — ưu tiên nó
--
-- "máy ép vỉ đến đâu" vẫn trượt. Lý do: chữ "may" một mình trúng 58 tên
-- thiết bị, vượt trần 25 nên cả loại ten_doi_tuong bị loại — kéo theo
-- mất luôn cụm "may ep vi" vốn trúng đúng MỘT cái.
--
-- Sửa bằng một luật đơn giản và đúng với cách người ta nói: trong cùng
-- một loại, giá trị nào được khớp bởi cụm DÀI HƠN thì cụ thể hơn, giữ
-- nó và bỏ phần khớp bởi cụm ngắn. "may ep vi" (7 ký tự) thắng "may"
-- (3 ký tự), nên chỉ còn Máy Ép Vỉ.
-- =====================================================================
...
begin
  ...
  for r in
    ...
    khop as (
      select d.loai, d.gia_tri, c.t as tu from tieng c join public.vmp_ai_tu_dien d on d.khoa like '%'||c.t||'%'
      union all
      select d.loai, d.gia_tri, c.t from cum c join public.vmp_ai_tu_dien d on d.khoa like '%'||c.t||'%'
      union all
      select d.loai, d.gia_tri, c.t from moi_tieng c join public.vmp_ai_tu_dien d
    ...
    -- Mỗi giá trị lấy cụm DÀI NHẤT đã khớp được nó
    xep as (select loai, gia_tri, max(length(tu)) as do_dai from khop group by loai, gia_tri),
    -- Trong mỗi loại, chỉ giữ nhóm khớp bởi cụm dài nhất
    dai_nhat as (select loai, max(do_dai) as md from xep group by loai)
    select x.loai, array_agg(distinct x.gia_tri) as ds, count(distinct x.gia_tri) as sl
    from xep x join dai_nhat d on d.loai = x.loai and x.do_dai = d.md
    group by x.loai
    having count(distinct x.gia_tri) <= 25
    order by count(distinct x.gia_tri)
