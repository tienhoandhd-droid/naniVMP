-- Hai vá cho vụ "My tháng 8": model bịa nguyên lịch 8 thiết bị + ngày.
--
-- Sự thật: My tháng 8 có 22 hạng mục (toàn tank, hạn đều 31/08). Câu trả
-- lời kể RO / nồi hơi / HVAC / tủ sấy... với ngày 05/08→28/08 — bịa 100%,
-- mà rpc_ai_kiem_chung vẫn chấm ty_le_bam=100 vì chỉ so CON SỐ trần:
-- "8", "15", "22"... trùng bừa bên trong các ngày ISO của ngữ cảnh.
--
-- Vá 1 — rpc_ai_thong_ke_loc hiểu chiều THỜI GIAN ("tháng 8", "tháng này",
-- "tuần này", "N ngày tới"): đếm đúng trong khoảng hạn VMP đó và nhét danh
-- sách TÊN THẬT vào câu kết luận. Bài học cũ: lời dặn thua sự hiện diện
-- của dữ liệu — có dữ liệu thật trong khối chốt thì model hết chỗ bịa.
--
-- Vá 2 — rpc_ai_kiem_chung soi thêm hai thứ model hay bịa nhất:
-- · NGÀY dd/mm không có trong dữ liệu (so cả dạng ISO lẫn DD/MM);
-- · TÊN in đậm không có trong dữ liệu lẫn từ điển tên thật vmp_ai_tu_dien.
-- Số nhỏ rời rạc vẫn dễ trùng bừa, nhưng bịa-lịch thì luôn kèm ngày + tên
-- nên hai lưới này bắt được đúng kiểu hỏng đã gặp. IMMUTABLE → STABLE vì
-- giờ cần đọc từ điển.

-- ═══════════════════ VÁ 1: LỌC THEO THỜI GIAN ═══════════════════
...
declare
  ...
  v_tu date; v_den date; v_nhan_tg text; v_m text[]; v_gom text; v_them integer;
  v_bo_qua  text[] := array[
    ...
    -- "cho tôi THÔNG TIN..." là cách mở đầu cực phổ biến, mà bỏ dấu thì
    -- "thông" trùng khít "Hệ THỐNG" — một câu vơ trọn 5 nhóm, 136 hạng mục.
    'thong','tin','thong tin','cho','ve','cua','xin',
    -- "lien QUAN" khớp "QUẦN áo" -> vơ luôn Tủ hấp tiệt trùng quần áo
...
begin
  ...
  if v_p like '% thang nay %' then
    ...
    if v_m is not null then
      ...
      if v_m is not null and v_m[1]::int between 1 and 12 then
        v_tu := make_date(coalesce((regexp_match(v_p, ' (20\d{2}) '))[1]::int,
                                   extract(year from current_date)::int), v_m[1]::int, 1);
        v_den := (v_tu + interval '1 month' - interval '1 day')::date;
        v_nhan_tg := format('tháng %s/%s', v_m[1]::int, extract(year from v_tu)::int);
      end if;
    end if;
  end if;
  ...
  for r in
    ...
    khop as (
      ...
      where (d.loai not in ('ten_doi_tuong','line')
         ...
         or btrim(d.kh) = c.t)
        -- "tiến độ CHUNG thế nào" không nói về line "Cân chung": chữ
        -- "chung" đứng một mình chỉ được nhận khi câu có đủ cụm "cân chung"
      ...
      from (select distinct t from unnest(string_to_array(btrim(v_q_dau),' ')) t
            where length(t) >= 2
              ...
              -- "tiến độ" chứa "tiến" nhưng cũng không — lớp phân tích viết
              -- lại câu rất hay chèn "tiến độ" nên cụm này bắt buộc phải chặn.
  ...
  loop
    ...
    -- Dính NHIỀU THIẾT BỊ KHÁC HỌ thì số gộp là số vô nghĩa, y như dính
    -- nhiều người. Thêm nhom_viec, khu_vuc, line: "nhóm hệ nước" dính cả
    -- nước tinh khiết lẫn nước thải, cộng gộp là con số không ai dùng được.
    ...
    if v_r.tong = 0 and coalesce(v_ho_tro,0) = 0 then
      -- Có lọc thời gian mà rỗng thì phải NÓI RÕ là rỗng — im lặng bỏ qua
      -- là đẩy mô hình vào chỗ không có dữ liệu, và nó sẽ bịa cho đầy.
    ...
    if r.loai = 'nguoi' and v_tu is not null then
      -- Có lọc thời gian: đếm trong khoảng, KHÔNG so với trung bình cả năm
      -- (so khoảng một tháng với mặt bằng cả năm là so lệch thước đo).
      v_ai := r.ds[1];
    ...
    -- Có lọc thời gian thì kể luôn TÊN THẬT các hạng mục trong khoảng —
    -- đây chính là thứ mô hình từng bịa khi trong tay không có gì.
    if v_tu is not null and v_r.tong > 0 then
      ...
      if v_gom is not null then
        v_nhan := v_nhan || format(' Gồm: %s.', v_gom)
                || case when (select count(distinct coalesce(o.name, i.object_code))
                              from public.vmp_plan_items i
                              left join public.vmp_objects o on o.code = i.object_code
                        ...
                        then ' (danh sách đã cắt còn 8 tên nhiều việc nhất)' else '' end;
      end if;
    end if;
...
begin
  ...
  -- 1. NGÀY dd/mm — thứ mô hình bịa nhiều nhất khi tự chế lịch. So với
  --    dữ liệu ở cả hai dạng: ISO (2026-08-31) và DD/MM trong câu chốt.
  for v_m in
  ...
  -- 2. CON SỐ — che ngày trước đã, kẻo 05/08 vỡ thành "05" và "08" rồi
  --    trùng bừa với ngày tháng ISO trong dữ liệu (vụ ty_le_bam=100 giả).
  ...
  -- 3. TÊN IN ĐẬM — format buộc in đậm tên thiết bị, nên tên bịa gần như
  --    chắc chắn nằm trong **...**. Tên phải có mặt trong dữ liệu đã đưa
  --    hoặc trong từ điển tên thật (vmp_ai_tu_dien đủ: người, thiết bị,
  --    nhóm việc, khu vực, line, mã, bộ phận).
  for v_cum in
  ...
  loop
    ...
    -- Một từ trơn không kèm số thì bỏ qua ("Tổng", "Nồi") — dễ oan;
    -- tên bịa thành hình luôn từ hai tiếng trở lên hoặc có mã số.
    continue when array_length(string_to_array(v_t, ' '), 1) < 2 and v_t !~ '[0-9]';
    -- Nhãn trình bày quen thuộc, không phải tên
    continue when v_t = any(array[
    ...
    select count(*) into v_treo from public.vmp_ai_tu_dien d
    where d.khoa like '%' || v_t || '%'
       or (length(d.khoa) >= 4 and (' ' || v_t || ' ') like '% ' || d.khoa || ' %');
    continue when v_treo > 0;
...
-- Ca hồi quy: đúng câu đã bịa lịch tháng 8 cho My (30/07/2026)
insert into public.vmp_ai_bo_kiem (ma, cau_hoi, mong_doi, ghi_chu) values
  ('loc_theo_thang',
   'My tháng 8 có quá nhiều việc không?',
   '{"duong":"thong_ke","chua_chuoi":["Tôn Nữ Thiện My","trong tháng 8/","Gồm:"],"khong_chua":["HVAC","trung bình mỗi người"]}'::jsonb,
   'Câu hỏi có chiều thời gian. Từng không lọc được tháng 8 nên model bịa nguyên lịch 8 thiết bị (RO, nồi hơi, HVAC...) kèm ngày — kiem_chung còn chấm 100 vì số nhỏ trùng bừa. Tầng thống kê phải đếm đúng trong tháng, kể tên thật, và không so với trung bình cả năm.')
