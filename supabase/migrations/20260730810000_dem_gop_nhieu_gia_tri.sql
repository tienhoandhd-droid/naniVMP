-- =====================================================================
-- Đếm được cả khi từ khoá trúng NHIỀU giá trị
--
-- Chạy thử 50 câu hỏi mô phỏng, 17 câu không khoanh được về đối tượng
-- nào. Tám câu trong đó trượt ĐÚNG (hỏi toàn nhà máy thì có gì mà
-- khoanh). Chín câu còn lại là lỗi thật, chia làm ba kiểu:
--
--  1. "Hằng có bận không" → trượt. Tên một tiếng "hằng" trùng với tiếng
--     "hạng" trong "hạng mục" nên nằm trong danh sách bỏ qua. Tên người
--     là tín hiệu quá mạnh để bị lọc chung với hư từ.
--
--  2. "bên QA còn bao nhiêu việc" → trượt. Mã bộ phận chỉ hai ký tự
--     (qa, qc, cd) mà ngưỡng đang là ba.
--
--  3. "line BFS đến đâu" → trượt. "bfs" trúng ba giá trị (BFS, BFS-W,
--     BFS-R) mà bản cũ đòi phải trúng ĐÚNG MỘT mới đếm. Đây mới là kiểu
--     đáng ngại nhất: người hỏi nói đúng tên nhóm, hệ nhận ra cả ba
--     dòng, rồi vẫn trả về rỗng — và AI quay sang lấy số tổng nhà máy,
--     tức là rơi lại đúng cái bug ban đầu.
--
-- Kiểu 3 chữa bằng cách GỘP: trúng nhiều giá trị thì đếm trên tất cả
-- rồi nói rõ đã gộp những gì. Đúng ý "hỏi một phần thì vẫn phải ra".
-- Chỉ bỏ khi trúng quá rộng (trên 25 giá trị) — lúc đó gộp lại thành
-- con số vô nghĩa, thà để AI hỏi lại cho hẹp.
-- =====================================================================
...
declare
  ...
  v_bo_qua  text[] := array[
    ...
    'hang','muc','tien','thiet','danh','sach','tinh','hinh','tat','ca',
...
begin
  ...
  -- Gom các giá trị trúng, theo từng loại. Ba đường bắt bù cho nhau:
  --   · đường thường: tiếng từ 3 ký tự, bỏ hư từ
  --   · đường TÊN NGƯỜI: không lọc hư từ, vì "Hằng" trùng tiếng "hạng"
  --   · đường MÃ NGẮN: đúng hai ký tự và trùng khít (qa, qc, cd, IQ, OQ)
  for r in
    with tieng as (
    ...
    moi_tieng as (
      select distinct t from unnest(string_to_array(btrim(v_q), ' ')) t where length(t) >= 3
    ),
    khop as (
      select d.loai, d.gia_tri from tieng c join public.vmp_ai_tu_dien d on d.khoa like '%'||c.t||'%'
      union
      select d.loai, d.gia_tri from cum   c join public.vmp_ai_tu_dien d on d.khoa like '%'||c.t||'%'
      union
      -- tên người: bắt cả khi tiếng đó nằm trong danh sách bỏ qua
      select d.loai, d.gia_tri from moi_tieng c join public.vmp_ai_tu_dien d
        on d.loai = 'nguoi' and d.khoa like '%'||c.t||'%'
      union
      -- mã ngắn hai ký tự: bộ phận và loại thẩm định
      select d.loai, d.gia_tri
      from (select distinct t from unnest(string_to_array(btrim(v_q), ' ')) t where length(t) = 2) c
      join public.vmp_ai_tu_dien d on d.loai in ('loai_td','bo_phan') and d.khoa = c.t
      union
      -- bí danh: "kcs" -> qc, "nước RO" -> hệ nước tinh khiết
      select b.loai, b.gia_tri from public.vmp_ai_bi_danh b
      where b.gia_tri is not null and (' '||v_q||' ') like '%'||b.bi_danh||'%'
    )
    select loai, array_agg(distinct gia_tri) as ds, count(distinct gia_tri) as sl
    from khop group by loai
    having count(distinct gia_tri) <= 25
    order by count(distinct gia_tri)
  ...
  loop
    ...
    if r.loai = 'nguoi' and r.sl = 1 and coalesce(v_tb_mo, 0) > 0 then
      ...
      v_nhan := format('%s CHỦ TRÌ %s việc chưa xong, trong khi trung bình mỗi người chủ trì %s việc. Vậy về tải việc, %s là %s.',
                       v_tb_ten, v_r.tong - v_r.hoan_thanh, v_tb_mo, v_tb_ten, lower(v_muc));
      if coalesce(v_ho_tro,0) > 0 then
        v_nhan := v_nhan || format(' Ngoài ra %s còn đứng tên HỖ TRỢ ở %s việc chưa xong — vai hỗ trợ nhẹ hơn chủ trì nên không cộng vào tải chính.', v_tb_ten, v_ho_tro);
      end if;
      v_nhan := v_nhan || case when v_r.qua_han > 0
        then format(' Còn chuyện trễ hạn thì tách biệt: %s đang có %s hạng mục quá hạn.', v_tb_ten, v_r.qua_han)
        else format(' Và %s không có hạng mục nào quá hạn.', v_tb_ten) end;
    elsif r.sl > 1 then
  ...
  return jsonb_build_object(
    ...
    'luu_y', 'Số liệu ĐẾM THẬT trên toàn bảng — dùng thẳng, đừng cộng trừ lại. '
             || 'so_gia_tri_da_gop > 1 nghĩa là đã GỘP nhiều giá trị: phải nói rõ đã gộp những gì. '
