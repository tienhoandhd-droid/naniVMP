-- Phân loại dữ liệu thành MỤC LỤC + gợi ý theo phân loại.
--
-- Ý người dùng: "phân loại dữ liệu -> gợi ý các mục chứa trong phần nào ->
-- gợi ý đơn giản hơn, tìm kiếm dễ hơn". Bốn việc:
--
-- 1. rpc_ai_muc_luc(): mục lục thật — nhóm việc / khu vực / bộ phận /
--    loại thẩm định kèm số lượng, và một câu mô tả dựng sẵn.
-- 2. rpc_ai_thong_ke_loc: câu LIỆT KÊ ("nhóm tank có những thiết bị nào")
--    kể tên thật; câu DANH MỤC không khoanh được ("có những nhóm nào")
--    trả nguyên mục lục vào khối chốt.
-- 3. rpc_ai_goi_y_chip(): chip gợi ý theo phân loại — hỏi lại thì chip là
--    đúng các lựa chọn; chạm một nhóm thì chip là thành viên nhóm; chạm
--    người thì chip theo người; chưa khoanh được thì chip là các nhóm lớn.
-- 4. rpc_ai_goi_y_tiep: nhánh chung chung cũng gợi theo nhóm lớn thật.
...
as $mucluc$
  with nv as (
    select coalesce(nullif(btrim(work_group), ''), '(chưa phân nhóm)') as ten,
           count(*) as tong, count(*) filter (where computed_status = 'over') as qua_han
  ...
  bp as (
    select coalesce(nullif(btrim(o.department), ''), '—') as ten, count(*) as tong
    from public.vmp_plan_items i join public.vmp_objects o on o.code = i.object_code
    where i.is_active group by 1 order by 2 desc),
  ltd as (
  ...
  select jsonb_build_object(
    ...
    'nhom_viec', (select jsonb_agg(jsonb_build_object('ten', ten, 'tong', tong, 'qua_han', qua_han)) from nv),
    'khu_vuc',   (select jsonb_agg(jsonb_build_object('ten', ten, 'tong', tong)) from kv),
    'bo_phan',   (select jsonb_agg(jsonb_build_object('ten', ten, 'tong', tong)) from bp),
    'loai_td',   (select jsonb_agg(jsonb_build_object('ten', ten, 'tong', tong)) from ltd),
    'mo_ta', format(
      'MỤC LỤC KẾ HOẠCH — tổng %s hạng mục. NHÓM VIỆC: %s. KHU VỰC: %s. BỘ PHẬN: %s. LOẠI THẨM ĐỊNH: %s. Kể đúng nguyên văn các tên này — không đổi tên, không thêm nhóm không có trong danh sách.',
      (select count(*) from public.vmp_plan_items where is_active),
      (select string_agg(ten || ' (' || tong || ')', ', ') from nv),
      (select string_agg(ten || ' (' || tong || ')', ', ') from kv),
      (select string_agg(ten || ' (' || tong || ')', ', ') from bp),
...
begin
  ...
  if e is not null then
    ...
    v_ten  := v_ds ->> 0;
    if v_loai in ('nhom_viec', 'khu_vuc', 'line', 'bo_phan', 'loai_td') then
...
-- Nhánh chung chung của gợi ý cũ cũng đi theo nhóm lớn thật
create or replace function public.rpc_ai_goi_y_tiep(p_hieu jsonb, p_year integer default null::integer)
 returns jsonb
 language plpgsql
 stable security definer
 set search_path to 'public'
...
-- Hai ca hồi quy cho đường danh mục
insert into public.vmp_ai_bo_kiem (ma, cau_hoi, mong_doi, ghi_chu) values
  ('muc_luc_nhom',
   'có những nhóm việc nào?',
   '{"duong":"thong_ke","chua_chuoi":["MỤC LỤC","Hệ thống tank"],"khong_chua":[]}'::jsonb,
   'Câu danh mục không khoanh về đối tượng nào — phải trả mục lục thật (tên nhóm + số lượng), không để mô hình tự kể tên nhóm theo trí nhớ.'),
  ('liet_ke_thanh_vien',
   'trong nhóm tank có những thiết bị nào?',
   '{"duong":"thong_ke","chua_chuoi":["Gồm:","Hệ thống tank"],"khong_chua":[]}'::jsonb,
   'Câu liệt kê thành viên một nhóm — phải kể tên thiết bị thật (Gồm:...), không kể là mô hình tự chế danh sách.')
on conflict (ma) do update set cau_hoi = excluded.cau_hoi, mong_doi = excluded.mong_doi, ghi_chu = excluded.ghi_chu;


-- ═══════════ VÁ THÊM SAU E2E: kiem_chung chặt hơn ═══════════
-- E2E bắt được câu bịa lọt lưới: "Hệ thống nước cất Pure Steam đạt 80%"
-- — tên lọt vì luật "cụm CHỨA tên thật thì quen" (nó chứa "nước cất");
-- số 80 lọt vì khớp chuỗi con của "180". Hai luật siết lại ở dưới.
