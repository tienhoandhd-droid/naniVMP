-- =====================================================================
-- Đồng bộ Sheet KHÔNG được đè lên dữ liệu nhập trên web
--
-- Từ 29/07/2026 Supabase là nơi lưu dữ liệu gốc, người dùng nhập trên
-- web. Nhưng hàm đồng bộ vẫn viết `status_vmp = excluded.status_vmp`,
-- `actual_vmp_date = excluded.actual_vmp_date`, `owner_name =
-- excluded.owner_name`… nghĩa là mỗi lần chạy nó chép ĐÈ toàn bộ 461
-- hạng mục bằng nội dung Sheet.
--
-- Đã dựng lại đúng tình huống trong transaction rồi rollback:
--   trước: owner_name='NGƯỜI NHẬP TRÊN WEB' · status_vmp='completed'
--          · actual_vmp_date='2026-07-29'
--   sau một lần đồng bộ: owner_name trống · status_vmp='not_started'
--          · actual_vmp_date NULL
-- Toàn bộ 461 dòng bị ghi lại. Hôm nay 29/07 đã có 5 lần đồng bộ chạy
-- thật, audit ghi nhận 844 lần owner_name bị đổi (218 hạng mục bị xoá
-- trắng tên QA, 436 hạng mục được điền lại).
--
-- Tệ hơn: chốt "bỏ qua nếu không đổi" trong SQL của n8n chỉ bỏ qua khi
-- checksum trùng VÀ không có bản ghi nào sửa sau lần sync trước. Tức là
-- HỄ CÓ NGƯỜI SỬA TRÊN WEB thì nó KHÔNG bỏ qua — chạy đè lên đúng thứ
-- vừa sửa.
--
-- Sửa: chia cột làm hai loại.
--   · Sheet vẫn làm chủ phần KẾ HOẠCH: deadline, phân loại báo cáo,
--     điểm trọng yếu, số ngày công, tần suất, tên/bộ phận đối tượng.
--   · Web làm chủ phần THỰC HIỆN: người thực hiện, ngày thực tế, trạng
--     thái 4 giai đoạn, lịch thẩm định. Đồng bộ chỉ ĐIỀN VÀO CHỖ TRỐNG,
--     không bao giờ đè giá trị đang có.
-- =====================================================================

CREATE OR REPLACE FUNCTION public.rpc_sync_vmp_sheet_snapshot(p_sheet_id text, p_sheet_gid text, p_tab_name text, p_headers jsonb, p_rows jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
