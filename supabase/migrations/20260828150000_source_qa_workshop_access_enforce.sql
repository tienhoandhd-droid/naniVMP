...
-- Task 3 needs the narrow rights-filtered object reader used by its business
-- proofs. Task 4 replaces this body with the fully inventoried query path.
create function public.rpc_list_source_objects(
...
begin
  ...
  select coalesce(jsonb_agg(to_jsonb(result_row)
           order by result_row.object_code,result_row.id),'[]'::jsonb)
    into v_rows
  from (
    select source_object.*
    from public.vmp_source_objects source_object
    where public.vmp_can_view_source_object(v_actor,source_object.id)
      and (source_object.is_active is true
           or (v_role in ('admin','qa_manager')
      ...
      and (coalesce(btrim(p_search),'')=''
           or source_object.object_code ilike '%'||btrim(p_search)||'%'
           or source_object.object_name ilike '%'||btrim(p_search)||'%')
      and (coalesce(p_filters,'{}'::jsonb)='{}'::jsonb or (
        (not (p_filters?'department')
         or source_object.department=p_filters->>'department')
        and (not (p_filters?'area_code')
         or source_object.area_code=p_filters->>'area_code')
        and (not (p_filters?'line')
         or source_object.line=p_filters->>'line')
      ))
      and (p_object_id is null or source_object.id=p_object_id)
      and (p_cursor is null or (source_object.object_code,source_object.id)>(
        p_cursor->>'object_code',(p_cursor->>'id')::uuid
      ))
    order by source_object.object_code,source_object.id
    limit p_limit
  ) result_row;
  return jsonb_build_object('ok',true,'rows',v_rows,'next_cursor',null);
...
begin
  ...
  if p_cursor is not null and (
       jsonb_typeof(p_cursor) is distinct from 'object'
       or nullif(p_cursor->>'department','') is null
       or nullif(p_cursor->>'area_code','') is null
  ...
  with choice as materialized (
    select distinct source_object.department,source_object.area_code,
           source_object.line
    from public.vmp_source_objects source_object
    where source_object.is_active
      ...
      and (coalesce(btrim(p_search),'')=''
           or coalesce(source_object.line,'') ilike
              '%'||btrim(p_search)||'%')
  ), returned as (
    select choice.*,
           row_number() over (
    ...
    )>(p_cursor->>'department',p_cursor->>'area_code',
       coalesce(p_cursor->>'line',''))
