-- =====================================================================
-- Chặn tiếng thường vơ nhầm tên riêng
--
-- Mở bộ câu hỏi vàng lên 109 câu thì lộ ngay một lớp lỗi chưa từng thấy:
-- tiếng tiếng Việt thông thường trùng với tên thiết bị.
--
--   "ai làm việc KÉM nhất"        -> khoanh vào  Kem/Gel (X1)
--   "thời tiết Hà NỘI hôm nay"    -> khoanh vào  Nồi hấp
--   "bộ PHẬN nào đuối nhất"       -> khoanh vào  Máy phân cực kế tự động
--   "nhiệt độ PHÒNG sạch"         -> khoanh vào  Laf A phòng máy
--   "tỷ lệ HOÀN THÀNH toàn nhà máy" -> khoanh vào Tào Tiến Hoàn
--
-- Toàn là câu KHÔNG được khoanh vào đâu cả, mà lại khoanh trúng một cái
-- máy ngẫu nhiên — rồi mô hình sẽ lấy số của cái máy đó mà trả lời. Đúng
-- kiểu sai nghe rất trôi chảy.
--
-- Ba lớp chặn:
--   1. Mở rộng danh sách hư từ bằng đúng những tiếng đã gây nhiễu.
--   2. Tên thiết bị và line phải khớp bằng tiếng >= 4 ký tự hoặc bằng
--      cụm — tiếng ba chữ cái quá chung để nhận diện một cái máy.
--   3. Tên người một tiếng bị bỏ khi nó nằm trong một cụm phổ thông
...
declare
  ...
  v_bo_qua  text[] := array[
    ...
    'toan','le','bo','phan','doi','lech','nha','may','viec','cong','tom','tat',
...
begin
  ...
  for r in
    ...
    khop as (
      ...
      where d.loai not in ('ten_doi_tuong','line') or length(c.t) >= 4
      union all
