 ...
 * ===================================================================== */
import { DEP_DAYS, SOON_DAYS, VMP_TODAY, PROG } from "../constants/vmp.js";
...
const YS = new Date(_yr, 0, 1);
const YE = new Date(_yr, 11, 31);
export const pctYear = (date) => clamp(((date - YS) / (YE - YS)) * 100, 0, 100);
...
export function tally(acts) {
  ...
  return {
    ...
    todo: acts.length - done - over,
    total: acts.length,
    rate: acts.length ? Math.round((done / acts.length) * 100) : 0,
  };
}
...
<p><b>Đơn vị:</b> V/Q Team — QLCL · <b>Nguồn:</b> Google Sheet (đồng bộ n8n)</p>
