...
/** Các cột căn giữa trong bảng danh mục. */
const HEAD_CENTER = new Set(["Nhóm", "Bộ phận", "Phụ trách", "Khu vực", "Mức ĐH", "Chu kỳ", "TĐ?", "Trạng thái"]);

function EditObjModal({ obj, isNew, onClose, onSave }: {
  obj: Partial<VmpObject>;
  isNew?: boolean;
  onClose: () => void;
  onSave: (obj: Record<string, unknown>, isNew?: boolean) => void;
}) {
  const [f, setF] = useState<Record<string, unknown>>(obj as Record<string, unknown>);
  const up = (k: string, v: unknown) => setF((s) => ({ ...s, [k]: v }));
  const sel = (k: string, opts: Array<{ v: string; l: string }>) => (
    <select value={String(f[k] ?? "")} onChange={(e) => up(k, e.target.value)} style={INP}>
  ...
  return (
    <Modal onClose={onClose} title={isNew ? "Thêm đối tượng" : "Sửa đối tượng"} icon={Pencil} wide>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
        <div style={FIELD}><span style={LBL}>Mã đối tượng</span><input style={{ ...INP, background: isNew ? "#fff" : C.pinkSoft }} value={String(f.code ?? "")} disabled={!isNew} onChange={(e) => up("code", e.target.value)} /></div>
        <div style={FIELD}><span style={LBL}>Nhóm</span>{sel("cls", Object.keys(CLS).map((k) => ({ v: k, l: (CLS as Record<string, { label: string }>)[k].label })))}</div>
        <div style={{ ...FIELD, gridColumn: "1 / -1" }}><span style={LBL}>Tên đối tượng</span><input style={INP} value={String(f.name ?? "")} onChange={(e) => up("name", e.target.value)} /></div>
        <div style={FIELD}><span style={LBL}>Bộ phận</span>{sel("dept", DEPTS.map((d) => ({ v: d.id, l: d.name })))}</div>
        <div style={FIELD}><span style={LBL}>Khu vực</span><input style={INP} value={String(f.area ?? "")} onChange={(e) => up("area", e.target.value)} /></div>
        <div style={FIELD}><span style={LBL}>Mức ảnh hưởng</span>{sel("crit", [{ v: "Cao", l: "Cao" }, { v: "TB", l: "Trung bình" }, { v: "Thấp", l: "Thấp" }])}</div>
        <div style={FIELD}><span style={LBL}>Tần suất (tháng)</span><input type="number" style={INP} value={String(f.freq ?? "")} onChange={(e) => up("freq", Number(e.target.value))} /></div>
        <div style={{ ...FIELD, gridColumn: "1 / -1" }}><span style={LBL}>Lý do / ghi chú</span><textarea rows={3} style={{ ...INP, resize: "vertical", fontWeight: 500 }} value={String(f.reason ?? "")} onChange={(e) => up("reason", e.target.value)} /></div>
      </div>
...
export default function InventoryView({ objects, acts, canEdit, onSave, onDelete }: {
  objects: VmpObject[];
  acts: Activity[];
  canEdit?: boolean;
  onSave?: (obj: Record<string, unknown>, isNew?: boolean) => void;
  ...
  const [edit, setEdit] = useState<{ obj: Partial<VmpObject>; isNew?: boolean } | null>(null);
  const [dept, setDept] = useState("all");
  const [person] = useState("");
  const [pscope] = useState("all");
  const ownerMap = useMemo(() => {
    const m: Record<string, { qa: Set<string>; other: Set<string> }> = {};
    acts.forEach((a) => {
      const r = (a._raw || {}) as Record<string, unknown>;
      if (!m[a.code]) m[a.code] = { qa: new Set(), other: new Set() };
      const qa = String(r.qa || a.owner || "").trim();
      const ot = String(r.ns_khac || "").trim();
      if (qa) m[a.code].qa.add(qa);
      if (ot) m[a.code].other.add(ot);
    });
    return m;
  }, [acts]);
  const matchPerson = (code: string): boolean => {
  ...
  const counts = Object.keys(CLS).reduce<Record<string, number>>((m, k) => {
    m[k] = objects.filter((o) => o.cls === k).length;
    return m;
  }, {});
  const ClsTab = ({ id, label, n }: { id: string; label: string; n: number }) => <button onClick={() => setCls(id)} style={{ padding: "8px 14px", borderRadius: 999, border: "none", cursor: "pointer", fontFamily: TEXT, fontSize: 12.5, fontWeight: 800, background: cls === id ? GRAD : C.pinkSoft, color: cls === id ? "#fff" : C.plumSoft, display: "flex", alignItems: "center", gap: 7 }}>{label}<span style={{ fontFamily: NUM, fontSize: 12, padding: "0 6px", borderRadius: 999, background: cls === id ? "rgba(255,255,255,.25)" : "#fff", color: cls === id ? "#fff" : C.plumSoft }}>{n}</span></button>;
  return (
      ...
      <Card>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 14 }}>
          ...
          {Object.keys(CLS).map((k) => (
            <ClsTab key={k} id={k}
              label={(CLS as Record<string, { label: string }>)[k].label}
              n={counts[k]} />
          ))}
        </div>
      ...
      <Card variant="strong">
        ...
        <div style={{ overflowX: "auto" }} className="vmp-scroll">
          <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: TEXT, minWidth: 880 }}>
            <thead><tr>{["Mã", "Tên / Lý do", "Nhóm", "Bộ phận", "Phụ trách", "Khu vực", "Mức ĐH", "Chu kỳ", "TĐ?", "Trạng thái", ""].map((h) => <th key={h} style={{ textAlign: HEAD_CENTER.has(h) ? "center" : "left", fontSize: 11, color: C.plumSoft, fontWeight: 800, letterSpacing: 0.5, padding: "0 12px 13px", textTransform: "uppercase", whiteSpace: "nowrap" }}>{h}</th>)}</tr></thead>
            <tbody>{filtered.map((o) => { const cl = (CLS as Record<string, typeof CLS.tb>)[String(o.cls ?? "tb")] ?? CLS.tb;
              const dp = DEPTS.find((d) => d.id === o.dept);
              const st = objStatus(o.code, acts);
              const ct = (CRIT as Record<string, typeof CRIT.TB>)[String(o.crit ?? "TB")] ?? CRIT.TB; const ow = ownerMap[o.code] || { qa: new Set(), other: new Set() }; const qaList = [...ow.qa].join(", "); return (
              <tr key={o.code} className="vmp-row" style={{ borderTop: `1px solid ${C.line}` }}>
                ...
                <td style={{ padding: "13px 12px", textAlign: "center", fontFamily: NUM, fontSize: 14, fontWeight: 800, color: C.plum }}>{Number(o.freq) > 0 ? o.freq + " tháng" : "—"}</td>
