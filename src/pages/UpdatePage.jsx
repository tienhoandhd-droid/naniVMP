...
import { stageOf, inPeriod } from "../utils/helpers.js";
import { toISO } from "../lib/n8nAdapter.js";
import { Card, CardTitle, Tag, Modal, Pill, ROField } from "../components/ui/Primitives.jsx";
  ...
  return (
    <Modal onClose={onClose} title="Cập nhật tiến độ" icon={Pencil} wide>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 22 }}>
        ...
        <button onClick={() => { onSave(act.id, f); onClose(); }} style={{ ...btnPrimary, flex: 2, padding: "12px", borderRadius: 13, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><Save size={17} /> Lưu tiến độ</button>
      </div>
    </Modal>
  );
}

export default function UpdateView({ acts, conn, isAdmin, onUpdate }) {
      ...
      <Card style={{ padding: 0, overflow: "hidden" }}>
        <div className="vmp-scroll" style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: TEXT, minWidth: 720 }}>
            ...
            <tbody>
              {list.map((a, i) => { const sg = STAGES.find((s) => s.id === stageByItem.get(a.id)); return (
