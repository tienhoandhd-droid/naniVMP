...
function mapClass(phanLoai: unknown): string {
  const x = lc(phanLoai);
...
function mapDept(boPhan: unknown): string {
  const x = lc(boPhan);
...
function critFromReportClass(v: unknown): string {
  const x = lc(v);
...
function critFromScore(v: unknown): string | null {
...
// completed/overdue).
function normStatus(v: unknown): "done" | "prog" | "todo" | "" {
  const x = lc(v);
  if (!x) return "";
 ...
 */
/** Một dòng thô từ webhook n8n / cột _raw của Supabase. */
type RawRow = Record<string, unknown>;

function deriveSt(row: RawRow, today: Date): "done" | "prog" | "over" | "todo" | "plan" {
  ...
  if (proto && proto.getTime() - today.getTime() > SOON) return "plan";
  return "todo";
}
 ...
 * ===================================================================== */
/** Hình dạng có thể nhận từ webhook n8n — mỗi node Respond trả một kiểu khác. */
type N8nPayload = {
  rows?: unknown[];
  objects?: unknown[];
  activities?: unknown[];
...
export function adaptFromN8n(
  payload: unknown,
): { objects: unknown[]; activities: unknown[]; [k: string]: unknown } {
  // Webhook n8n có thể trả về MẢNG (tuỳ cấu hình node Respond) → tự bóc cho đúng.
  let p: N8nPayload;
  if (Array.isArray(payload)) {
    const first = payload[0] as N8nPayload | undefined;
    p = (payload.length && first && Array.isArray(first.rows))
      ? first                        // [{ ok, count, rows }]
      : { rows: payload };           // [ {dòng}, {dòng}, ... ]
  } else {
    p = (payload || {}) as N8nPayload;
  }
  // Tương thích ngược: nếu đã đúng định dạng app thì trả về luôn.
  if (Array.isArray(p.objects) || Array.isArray(p.activities)) {
    return {
      objects: p.objects || [],
  ...
  const rows: RawRow[] = Array.isArray(p.rows) ? (p.rows as RawRow[]) : [];
  ...
  for (const r of rows) {
    ...
    const score = parseInt(s(r.diem_trong_yeu), 10);
    const effort = parseFloat(s(r.so_ngay_cong));
    ...
    if (!objMap.has(code)) {
      objMap.set(code, {
        ...
        gxp: "GxP",
        crit,
        freq: parseInt(s(r.tan_suat), 10) || 12,
        need,
        reason: "",
      });
    } else {
...
export function deriveActivityFields(raw: RawRow): {
  st: "done" | "prog" | "over" | "todo" | "plan";
  docDone: boolean;
  target: string;
} {
  const today = new Date();
 ...
 * ===================================================================== */
export function buildPing(): { action: string; ts: number } {
...
export async function fetchVmpData(readUrl: string, force = false): Promise<unknown> {
...
export function clearVmpCache(): void {
