...
export async function activateAccount(args: ActivationArgs): Promise<{ kind: "verified" | "stale" | "rejected" | "written_unverified" | "busy"; message?: string }> {
  ...
  try { const row = await args.reload(args.userId); return args.isCurrent() && row ? { kind: "verified" } : { kind: "written_unverified", message: "Đã ghi nhưng chưa đối chiếu lại được." }; } catch (error) { return args.isCurrent() ? { kind: "written_unverified", message: `Đã ghi nhưng chưa đối chiếu lại được: ${errorMessage(error)}` } : { kind: "stale" }; }
}
...
export function AccountAdministrationPanel({ canManageAccounts, revision = 0, loaders = defaultLoaders, mutateActive = setUserActive, onEditRole }: AccountAdministrationPanelProps) {
  ...
  const reload = useCallback(async (userId: string) => { const next = await loadAccountAdministrationSnapshot(loaders); setSnapshot(next); return next.rows.find((row) => row.userId === userId) ?? null; }, [loaders]);
  const [draft, setDraft] = useState<{ row: AccountAdministrationRow; next: boolean; reason: string; token: number } | null>(null); const activeGeneration = useRef(0); const [status, setStatus] = useState<string | null>(null); const [submitting, setSubmitting] = useState(false);
  const confirm = async () => { const userId = draft?.row.userId; if (!userId || submitting || !draft) return; setSubmitting(true); const target = draft; const outcome = await activateAccount({ userId, nextActive: target.next, reason: target.reason.trim(), mutate: mutateActive, reload, isCurrent: () => activeGeneration.current === target.token }); if (outcome.kind === "verified" && activeGeneration.current === target.token) setDraft(null); else if (outcome.kind !== "stale") setStatus(outcome.message || "Không thể xác minh trạng thái"); setSubmitting(false); };
