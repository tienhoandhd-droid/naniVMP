...
import {
  initialSourceQaCandidatesState, initialSourceQaRequestFence, invalidateSourceQaRequestFence,
  reduceSourceQaCandidates, sourceQaRequestIsCurrent,
...
export function useSourceQaCandidates(currentIds: readonly string[]) {
  ...
  const requestFence = useRef(initialSourceQaRequestFence(includeKey));
  ...
  const load = useCallback(async ({ search, append }: { search: string; append: boolean }) => {
    const scope = requestFence.current;
    ...
    if (!mounted.current || !sourceQaRequestIsCurrent(requestFence.current, scope)) return;
  ...
  useEffect(() => {
    ...
    // A scheduled result from the prior ID set must not become current while
    // React batches this effect's state update.
    requestId.current += 1;
