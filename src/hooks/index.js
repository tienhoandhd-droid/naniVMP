...
import { fetchVmpDataFromSupabase, updateProgressSupabase, upsertObjectSupabase, pushToSheet } from "../lib/supabaseData.js";
...
export function useVmpData() {
  ...
  const updateActivity = useCallback(async (id, patch, userName, reason) => {
    ...
    try {
      ...
      const result = await updateProgressSupabase(id, supabasePatch, reason);
    ...
    if (conn.writeUrl) {
      ...
      if (sheetRes.ok) {
        ...
        // Supabase OK nhưng Sheet lỗi → CẢNH BÁO: có thể bị ghi đè ở lần sync sau
        setSaveStatus("warning");
        setConn((c) => ({
          ...c,
          msg: `⚠ Đã lưu Supabase nhưng CHƯA đồng bộ Google Sheet (${sheetRes.error || "lỗi"}). ` +
               `Cập nhật có thể bị ghi đè ở lần sync sau — vui lòng thử lại hoặc sửa trực tiếp trên Sheet.`,
        }));
        setTimeout(() => setSaveStatus(""), 8000);
        return { ok: true, sheetFailed: true };
      }
    } else {
